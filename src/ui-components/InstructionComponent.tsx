...
export const InstructionComponent: React.FC<InstructionComponentProps> = ({
  ...
  const renderContent = () => (
    <div className="relative overflow-hidden">
      ...
      >
        <div className="flex items-start gap-3 p-4 border-b border-gray-100 bg-white">
          <div className="flex-shrink-0 w-8 h-8 bg-blue-500 text-white rounded-full flex items-center justify-center text-sm font-medium">
            {instruction.order}
          </div>
          <div className="flex-1">
            <div className={`text-gray-900 ${instruction.markedDone ? 'line-through text-gray-400' : ''}`}>
              {instruction.instruction}
            </div>
          </div>
      ...
      {showOptionsMenu && (
        <div className="fixed bg-white border rounded-lg shadow-lg py-1 min-w-[120px] z-[9999]"
          ...
          >
            ...
            {instruction.markedDone ? 'Mark Undone' : 'Mark Done'}
          </button>
        </div>
      )}
    </div>
